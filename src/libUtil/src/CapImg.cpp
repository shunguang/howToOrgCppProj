#include "libUtil/include/CapImg.h"
#if APP_USE_CV
using namespace std;
using namespace app;
...
, m_isColor(isColor_)
{
    appLog("CapImg::CapImg(): cap images from folder: %s", m_dir.c_str() );
    probe();
    appLog( "isProbeSuccess()=%d", isProbeSuccess());
}
...
{
    appLog("CapImg::CapImg(): cap image from video file:%s", m_videoFile.c_str() );
    m_cvCap = std::make_shared<cv::VideoCapture>(videoFile_);
    if (!m_cvCap->isOpened()){
        m_cvCap = nullptr;
        //error in opening the video input
        appLog("CapImg::CapImg(): video file cannot be opened!");
    }
    else{
      probe();
    }
...
bool CapImg::getNextFrame(cv::Mat &I)
{
  if( m_isFromVideoFile ){
    ...
    appLog( "curImgFile=%s, m_isColor=%d", curImgFile.c_str(), m_isColor);

    I = cv::imread(curImgFile, m_isColor ? cv::IMREAD_COLOR : cv::IMREAD_COLOR);
    if (I.cols != m_imgW || I.rows != m_imgH){
      appLog("apImg::getNextFrame(): wrong iamge Size");
    }
  }
...
void CapImg::probe()
{
  ...
  appLog( "CapImg::probe(): m_nTotFrames=%d, m_imgW=%d, m_imgH=%d, m_nextFrmIdx=%d\n", m_nTotFrames, m_imgW, m_imgH, m_nextFrmIdx) ;
}
